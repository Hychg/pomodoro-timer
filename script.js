...
function startTimer(timerDisplay) {
    ...
    resetBtn.addEventListener("click", () => {
        if (!currentTimer) {
            timerMsg.textContent = "Pilih timer terlebih dahulu!";
            timerMsg.style.display = "block";
            return;
        }
        ...
        if (currentTimer.id === "pomodoro-timer") {
            defaultDuration = settings.pomodoro;
        } else if (currentTimer.id === "short-timer") {
            defaultDuration = settings.shortBreak;
        } else if (currentTimer.id === "long-timer") {
            defaultDuration = settings.longBreak;
        }
        
        currentTimer.setAttribute("data-duration", `${defaultDuration}:00`);
        const timeElement = currentTimer.querySelector(".time");
        if (timeElement) {
            timeElement.textContent = `${defaultDuration}:00`;
        }
        timerMsg.style.display = "none";
    })
